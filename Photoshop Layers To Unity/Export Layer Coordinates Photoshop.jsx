...
function recurseLayers(currLayers) {
  ...
  var bottom = 0;
  var name = "";
  for ( var index = 0; index < currLayers.layers.length; index++ ) {
    layerRef = currLayers.layers[index];
    left = layerRef.bounds[0].value;
    top = layerRef.bounds[1].value;
    right = layerRef.bounds[2].value;
    bottom = layerRef.bounds[3].value;
    name = layerRef.name;
    coords += index + "," +  layerRef.name + "," + left + "," + top + "," + right+"," + bottom+"|";
    //test if it's a layer set
    if ( isLayerSet(currLayers.layers[index]) ) {
      recurseLayers(currLayers.layers[index]);
    }
  }
}
